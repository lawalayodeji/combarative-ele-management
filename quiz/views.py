    ...
    try:
        ...
        if user.is_teacher == True:
            if request.method == 'POST':
                ...
                return render(request, 'quiz/quiz.html', {'course': course, 'faculty': Student.objects.get(user_id = user.id),
                'myId': user.id,
                })
    ...
    try:
        ...
        if user.is_teacher == True:
            ...
            if request.method == 'POST':
                ...
                return render(request, 'quiz/addQuestion.html', {'course': course, 'quiz': quiz, 'faculty': Student.objects.get(user_id = user.id),
                  'myId': user.id,
    ...
    if user.is_teacher == True:
        ...
        for quiz in quizzes:
            ...
            if quiz.start < datetime.datetime.now():
                ...
                quiz.started = False
            quiz.save()
        return render(request, 'quiz/allQuizzes.html', {'course': course, 'quizzes': quizzes, 'faculty': Student.objects.get(user_id = user.id), 
        'myId': user.id,    
        })
    else:
        return redirect('std_login')
...
def startQuiz(request, code, quiz_id):
    ...
    if user.is_teacher == False:
        ...
        return render(request, 'quiz/portalStdNew.html', {'course': course, 'quiz': quiz, 'questions': questions, 'total_questions': total_questions, 'student': Student.objects.get(user_id = user.id),
        'myId': user.id,    
        })
    else:
        return redirect('std_login')
...
def quizSummary(request, code, quiz_id):
    ...
    if user.is_teacher == True :
        ...
        context = {'course': course, 'quiz': quiz, 'questions': questions, 'time': time, 'total_students': total_students,
                   'students': students, 'faculty': Student.objects.get(user_id = user.id), 
                    'myId': user.id,
                   }
